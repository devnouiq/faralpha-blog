...
import type { SystemStatus, Candidate, Position, StopAlert, RegimeData, BacktestData, WsEvent, BuyAction, SignalsResponse, TradingStatus } from './types'
...
export default function App() {
  const [activeTab, setActiveTab] = useState<'momentum' | 'intraday'>('momentum')
  ...
  const [scanProgress, setScanProgress] = useState<{ step: string; message: string } | null>(null)
  ...
  useEffect(() => {
    const ws = api.connectWs((evt: WsEvent) => {
      ...
      if (evt.type === 'buy_signal') {
        ...
        setToast({ type: 'buy', message: `BUY signal: ${ticker} — check Signals table` })
        setTimeout(() => setToast(null), 10000)
      }
      if (evt.type === 'sell_signal') {
        const ticker = evt.data?.ticker || 'Unknown'
        setToast({ type: 'sell', message: `STOP HIT: ${ticker} at ${evt.data?.stop_price?.toFixed(2)} — sell at next open` })
  ...
  const handleSync = async (mkt: string) => {
    setBusy(true); setError(null); setScanProgress({ step: 'sync', message: 'Syncing prices…' })
    try {
      await api.triggerSync(mkt, true)
      await refresh()
    } catch (e: any) { setError(e.message) }
    finally { setBusy(false); setScanProgress(null) }
  }

  const handlePipeline = async (mkt: string) => {
    setBusy(true); setError(null); setScanProgress({ step: 'pipeline', message: 'Running pipeline…' })
    try {
  ...
  const handleScan = async (mkt: string) => {
    setBusy(true); setError(null); setScanProgress({ step: 'sync', message: 'Starting scan…' })
    try {
  ...
  const handleUniverse = async () => {
    setBusy(true); setError(null); setScanProgress({ step: 'cleanup', message: 'Refreshing universe…' })
    try {
      ...
      if (r.purged) {
        for (const [mkt, info] of Object.entries(r.purged) as [string, any][]) {
          if (info.delisted_removed > 0) parts.push(`Purged ${info.delisted_removed} delisted`)
        }
      }
      if (r.india_active) parts.push(`${r.india_active} active`)
      setToast({ type: 'buy', message: `Universe refreshed! ${parts.join(' · ')}` })
      setTimeout(() => setToast(null), 10000)
      await refresh()
    } catch (e: any) { setError(e.message) }
    finally { setBusy(false); setScanProgress(null) }
  ...
  const handleAddPosition = async (pos: { ticker: string; market: string; entry_date: string; entry_price: number; shares: number; notes?: string }) => {
    try {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100">
      ...
      {toast && (
        <div className={`mx-4 mt-2 p-3 rounded-lg text-sm font-semibold flex items-center justify-between animate-pulse ${
          toast.type === 'buy'
      ...
      <div className="flex items-center justify-between px-4 pt-4">
        ...
        <div className="flex items-center gap-2">
          <label className="text-xs text-slate-500" title="Enter your total portfolio value. This is used to calculate position sizes (₹ per trade, number of shares) and risk amounts for each buy signal.">
          ...
          />
          {capital > 0 && (
            <span className="text-xs text-slate-600">→ position sizes shown in signals</span>
          )}
        </div>
      </div>
