...
export default function IntradayPanel() {
  ...
  /** True after Full Sync returns `started` until `intraday_sync_complete` (or request error). */
  const intradaySyncPendingRef = useRef(false)
  ...
  useEffect(() => {
    const ws = api.connectWs((evt: any) => {
      if (
        evt.type === 'scan_progress' &&
        intradaySyncPendingRef.current &&
        evt.data?.message
      ) {
        setSyncMsg(evt.data.message as string)
      }
      if (evt.type === 'intraday_sync_complete') {
        intradaySyncPendingRef.current = false
        setSyncing(false)
        const d = evt.data || {}
        if (d.status === 'ok') {
          const n = d.watchlist_count ?? 0
          setSyncMsg(`Done — ${n} watchlist stock${n !== 1 ? 's' : ''}`)
          refreshStatic().catch(() => {})
          setError(null)
        } else {
          setError(d.error || 'Intraday sync failed')
          setSyncMsg(null)
        }
  ...
  const handleSync = async () => {
    setSyncing(true)
    setSyncMsg('Starting full sync…')
    setError(null)
    intradaySyncPendingRef.current = true
    try {
      const res = await api.syncIntraday()
      if (res.status === 'started') {
        setSyncMsg(res.message || 'Sync running — this can take many minutes.')
        return
      }
      if (res.status === 'ok') {
        intradaySyncPendingRef.current = false
                  ...
                  <tr key={`${t.ticker}-${t.order_date}-${i}`} className={`border-b border-slate-800/50 hover:bg-slate-800/30 ${
                    ...
                    <td className={`py-1.5 px-2 text-right font-mono text-xs ${isOpen ? 'text-violet-400' : 'text-slate-500'}`}>
                      {t.exit_date ? t.exit_date.slice(5)
                        : isOpen && t.order_date ? (() => {
                            const hold = t.max_hold_days || 10
                            const d = new Date(t.order_date)
                            let added = 0
                            while (added < hold) { d.setDate(d.getDate() + 1); if (d.getDay() !== 0 && d.getDay() !== 6) added++ }
