...
def _append_benchmark_from_df(con, df: pd.DataFrame) -> None:
    if use_postgres_database():
        from psycopg2.extras import execute_values

        raw = getattr(con, "_raw", con)
        tuples = [tuple(x) for x in df[["date", "ticker", "close"]].to_numpy()]
        sql = """
            INSERT INTO benchmark (date, ticker, close) VALUES %s
...
def _insert_prices_ignore(con, df: pd.DataFrame) -> None:
    """DuckDB ``INSERT OR IGNORE … FROM df`` vs PG ``ON CONFLICT DO NOTHING``."""
    if use_postgres_database():
        from psycopg2.extras import execute_values
...
def _download_benchmark(con, market: str) -> None:
    ...
    try:
        ...
        _append_benchmark_from_df(con, df)
        total = con.execute(
                ...
                try:
                    ...
                    if _detect_scale_jump(df, con, ticker, mkt):
                        ...
                        if not df_full.empty:
                            _insert_prices_ignore(con, df_full)
