...
def run(market: str = "both") -> None:
    ...
    _upsert_by_market(con, "regime", combined, markets)
    log.info(f"✓ Regime table: {len(combined):,} rows")
    con.close()


def _upsert_by_market(con, table: str, df, markets: list[str]) -> None:
    ...
    if old_col_names == new_col_names:
        ...
        con.execute(f"CREATE TABLE {table} AS SELECT * FROM df")
        if other_count > 0:
            log.warning(f"Schema change on '{table}': {other_count:,} rows from "
