...
async def _run_full_scan_background(req: ScanRequest) -> None:
    """Runs after HTTP returns so nginx/Azure gateways do not time out (504)."""
    try:
        async with op_lock:
            result = await asyncio.get_event_loop().run_in_executor(
            ...
            log.info(
                "POST /api/scan (background) complete — signals=%d  stop_alerts=%d  errors=%d",
        ...
        log.exception("Background full scan failed: %s", e)
...
async def full_scan(req: ScanRequest):
    ...
    Returns immediately with ``status: started`` so reverse proxies do not return
    504 Gateway Timeout while work runs (often 5–30+ minutes). Progress and
    completion are pushed over the WebSocket (``scan_progress``, ``scan_complete``).
    """
    if op_lock.locked():
        log.warning("Scan rejected — another operation is in progress")
        raise HTTPException(409, "Another operation is in progress")
    log.info("POST /api/scan  market=%s  force=%s (background)", req.market, req.force)
    asyncio.create_task(_run_full_scan_background(req))
    return {
        "status": "started",
        "message": "Full scan started — progress updates on this page; can take many minutes.",
    }
