...
from faralpha.api.state import broadcast, broadcast_from_thread, op_lock, table_exists
...
async def _run_intraday_sync_background() -> None:
    """Runs after HTTP returns so reverse proxies do not return 502/504 during long work."""
    try:
        async with op_lock:
            broadcast_from_thread(
                "scan_progress",
                {"step": "intraday_sync", "message": "Syncing daily prices via Kite…"},
            )
            ...
            broadcast_from_thread("scan_progress", {"step": "done", "message": "Intraday sync complete"})
            await broadcast(
                "intraday_sync_complete",
                {
                    "status": "ok",
        ...
        log.exception("Intraday sync failed: %s", e)
        broadcast_from_thread("scan_progress", {"step": "done", "message": "Intraday sync failed"})
        await broadcast(
            "intraday_sync_complete",
...
async def intraday_sync():
    ...
    Returns immediately with ``status: started`` so gateways do not time out (502/504)
    while work runs (often many minutes). Progress uses ``scan_progress`` on the WebSocket;
    completion is ``intraday_sync_complete``.
    """
    if op_lock.locked():
        log.warning("Intraday sync rejected — another operation is in progress")
        raise HTTPException(409, "Another operation is in progress")
    log.info("POST /api/intraday/sync (background)")
    asyncio.create_task(_run_intraday_sync_background())
    return {
        "status": "started",
        "message": "Intraday full sync started — watch progress here; can take many minutes.",
    }
...
async def intraday_status():
    ...
    if use_postgres_database():
        try:
            ...
            rows = c.execute(
                "SELECT interval, COUNT(*), COUNT(DISTINCT ticker), MIN(ts), MAX(ts) "
                "FROM candles GROUP BY interval"
            ).fetchall()
            c.close()
            for ivl, cnt, tickers, mn, mx in rows:
                scrape[ivl] = {
                    "candles": cnt,
                    "tickers": tickers,
                    "from": str(mn) if mn else None,
                    "to": str(mx) if mx else None,
                    "source": "postgres",
                }
        except Exception:
            pass
        ...
        for interval, db_path in config.INTRADAY_DB_PATHS.items():
            try:
                ...
                row = c.execute(
                    "SELECT COUNT(*), COUNT(DISTINCT ticker), MIN(ts), MAX(ts) FROM candles"
                ).fetchone()
                c.close()
                scrape[interval] = {
                    "candles": row[0],
