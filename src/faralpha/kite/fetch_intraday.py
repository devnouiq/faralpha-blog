...
from faralpha.kite.intraday_db import get_conn as get_intraday_conn, init_schema as init_intraday_schema
...
def fetch_intraday_chunked(
    ...
    if tickers is None:
        ...
        if _cache.exists():
            ...
            mcon = _ddb.connect(str(config.DB_PATH), read_only=True)
            rows = mcon.execute(
                "SELECT ticker FROM stocks WHERE market = 'india'"
            ).fetchall()
            tickers = [r[0] for r in rows]
